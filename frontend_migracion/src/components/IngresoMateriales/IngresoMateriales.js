  ...
  const renderHistorial = () => {
    return (
      <div className="historial-container">
        ...
        {/* FILTROS */}
        <div className="ingreso-card" style={{ marginBottom: '25px' }}>
          <div className="ingreso-card-header">
            <span className="ingreso-icon">🔍</span>
            <h3 className="ingreso-card-title">Filtros de Búsqueda</h3>
          </div>
          <div className="ingreso-card-body">
            <div className="ingreso-form-row">
              
              <div className="ingreso-form-group">
                <label className="ingreso-form-label">📅 Desde</label>
                <input
                  type="date"
                  className="ingreso-form-input"
                  value={fechaDesde}
              ...
              <div className="ingreso-form-group">
                <label className="ingreso-form-label">📅 Hasta</label>
                <input
                  type="date"
                  className="ingreso-form-input"
                  value={fechaHasta}
              ...
              <div className="ingreso-form-group" style={{ display: 'flex', alignItems: 'flex-end', gap: '10px' }}>
                <button
                  className="ingreso-btn ingreso-btn-primary"
        ...
        {/* TABS INTERNOS PARA TIPOS DE HISTORIAL */}
        <div className="ingreso-tabs-historial">
          <button
            className={`ingreso-tab-historial ${tabHistorial === 'oc' ? 'activo' : ''}`}
            onClick={() => setTabHistorial('oc')}
          >
            📦 Ingresos de Materiales (OC)
            {historialIngresos.length > 0 && (
              <span className="tab-badge">{historialIngresos.length}</span>
            )}
          </button>
          <button
            className={`ingreso-tab-historial ${tabHistorial === 'os' ? 'activo' : ''}`}
            onClick={() => setTabHistorial('os')}
          >
            🔧 Conformidades de Servicio (OS)
            {historialServicios.length > 0 && (
              <span className="tab-badge">{historialServicios.length}</span>
            )}
          </button>
          <button
            className={`ingreso-tab-historial ${tabHistorial === 'id' ? 'activo' : ''}`}
            onClick={() => setTabHistorial('id')}
          >
            📥 Ingresos Directos (ID)
            {historialDirectos.length > 0 && (
              <span className="tab-badge">{historialDirectos.length}</span>
            )}
          </button>
        </div>
        ...
        {tabHistorial === 'oc' && (
          <div className="ingreso-card">
            <div className="ingreso-card-header" style={{
              background: 'linear-gradient(135deg, #11998e 0%, #38ef7d 100%)'
            }}>
              <span className="ingreso-icon">📦</span>
              <h3 className="ingreso-card-title">
                Historial de Ingresos de Materiales (OC)
              </h3>
            </div>
            <div className="ingreso-card-body">
              {historialIngresos.length > 0 ? (
                    ...
                    <tbody>
                      {historialIngresos.map((ingreso, index) => (
                        <tr key={index}>
                          ...
                          <td>{ingreso.proyecto_almacen}</td>
                          <td>
                            <span style={{
                              padding: '4px 12px',
                              borderRadius: '12px',
                              fontSize: '12px',
                              fontWeight: '600',
                              background: ingreso.estado === 'COMPLETADA' 
                ...
                <div style={{ textAlign: 'center', padding: '40px', color: '#999', fontSize: '16px' }}>
                  📭 No hay registros de ingresos de materiales
                </div>
              )}
            </div>
          </div>
        ...
        {tabHistorial === 'os' && (
          <div className="ingreso-card">
            <div className="ingreso-card-header" style={{
              ...
              <span className="ingreso-icon">🔧</span>
              <h3 className="ingreso-card-title">
                Historial de Conformidades de Servicio (OS)
              </h3>
            </div>
            <div className="ingreso-card-body">
              {historialServicios.length > 0 ? (
                  ...
                  <table className="ingreso-table">
                    ...
                    <tbody>
                      {historialServicios.map((servicio, index) => (
                        <tr key={index}>
                          <td style={{ fontWeight: '600', color: '#f5576c' }}>
                            {servicio.id_conformidad}
                          </td>
                          <td>{servicio.correlativo}</td>
                          <td>{new Date(servicio.fecha_conformidad).toLocaleDateString()}</td>
                          <td>{servicio.proveedor}</td>
                          <td>{servicio.proyecto_almacen}</td>
                          <td>
                            <span style={{
                              padding: '4px 12px',
                              borderRadius: '12px',
                              fontSize: '12px',
                              fontWeight: '600',
                              background: servicio.estado === 'COMPLETADA' 
                                ? 'linear-gradient(135deg, #11998e 0%, #38ef7d 100%)'
                                : 'linear-gradient(135deg, #f2994a 0%, #f2c94c 100%)',
                              color: 'white'
                            }}>
                          ...
                          <td>{servicio.num_doc_servicio || '-'}</td>
                ...
                <div style={{ textAlign: 'center', padding: '40px', color: '#999', fontSize: '16px' }}>
                  📭 No hay registros de conformidades de servicio
                </div>
              )}
            </div>
          </div>
        ...
        {tabHistorial === 'id' && (
          <div className="ingreso-card">
            <div className="ingreso-card-header" style={{
              ...
              <span className="ingreso-icon">📥</span>
              <h3 className="ingreso-card-title">
                Historial de Ingresos Directos (ID)
              </h3>
            </div>
            <div className="ingreso-card-body">
              {historialDirectos.length > 0 ? (
                <div style={{ overflowX: 'auto' }}>
                  <table className="ingreso-table">
                    ...
                    <tbody>
                      {historialDirectos.map((directo, index) => (
                        <tr key={index}>
                          <td style={{ fontWeight: '600', color: '#764ba2' }}>
                            {directo.id_ingreso}
                          </td>
                          <td>{new Date(directo.fecha_ingreso).toLocaleDateString()}</td>
                          <td>{directo.empresa}</td>
                          <td>{directo.proveedor}</td>
                          <td>{directo.proyecto_almacen}</td>
                          <td style={{ textAlign: 'right', fontWeight: '700', color: '#11998e' }}>
                            {parseFloat(directo.total || 0).toFixed(2)}
                          </td>
                          <td style={{ textAlign: 'center', fontWeight: '600' }}>
                            {directo.moneda}
                          </td>
                          <td>{directo.usuario}</td>
                          <td>{directo.num_guia || '-'}</td>
                          <td>{directo.factura || '-'}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                ...
                <div style={{ textAlign: 'center', padding: '40px', color: '#999', fontSize: '16px' }}>
                  📭 No hay registros de ingresos directos
                </div>
              )}
            </div>
          </div>
  ...
  return (
    <div className="ingreso-materiales-container">
      ...
      <div className="ingreso-tabs">
        ...
        <button
          className={`ingreso-tab ${tabActivo === 'historial' ? 'activo' : ''}`}
