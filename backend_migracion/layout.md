...
import GestionHorarios from '../RecursosHumanos/GestionHorarios';
...
import ProtectedRRHH from '../RecursosHumanos/ProtectedRRHH';
...
const Layout = ({ onLogout, user: propUser }) => {
  ...
  const renderContent = () => {
    switch (activeModule) {
      ...
      case 'gestion-horarios-rrhh':
      case 'reportes-horarios-rrhh':
      case 'gestion-usuarios-rrhh':
      case 'dashboard-admin-rrhh':
      case 'vacantes':
      case 'postulantes':
        // Todas las vistas de RRHH protegidas con una sola autenticación
        return (
          <ProtectedRRHH onCancelar={() => {
            handleModuleChange('dashboard');
            // Emitir evento para que el Sidebar cambie a LOGÍSTICA
            window.dispatchEvent(new CustomEvent('cambiarModuloSidebar', { detail: { modulo: 'logistica' } }));
          }}>
            {activeModule === 'dashboard-supervisor' && <DashboardSupervisor />}
            {activeModule === 'reportes-asistencia-rrhh' && <ReportesAsistencia />}
            {activeModule === 'reportes-emergencias-rrhh' && <ReportesEmergencias />}
            {activeModule === 'gestion-horarios-rrhh' && <GestionHorarios />}
            {activeModule === 'reportes-horarios-rrhh' && <ReportesHorarios />}
            {activeModule === 'gestion-usuarios-rrhh' && <GestionUsuarios />}
            {activeModule === 'dashboard-admin-rrhh' && <DashboardAdministrador />}
            {activeModule === 'vacantes' && <Vacantes />}
            {activeModule === 'postulantes' && <Postulantes />}
          </ProtectedRRHH>
        );
      default:
        return <DashboardContent />;
...
const getModuleTitle = (moduleId) => {
  const titles = {
    ...
    'gestion-horarios-rrhh': 'Gestión de Horarios',
