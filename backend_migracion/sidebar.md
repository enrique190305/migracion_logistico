...
const Sidebar = ({ isCollapsed, onToggle, activeModule, onModuleChange, isAdmin, user, onMouseEnter, onMouseLeave }) => {
      ...
      subcategories: [
        ...
        },
        {
          ...
          items: [
            ...
            { id: 'gestion-horarios-rrhh', title: 'Gestión de Horarios', icon: '🕐' }
          ]
        },
        {
  ...
  // Escuchar evento para cambiar módulo del sidebar desde fuera
  useEffect(() => {
    const handleCambiarModuloSidebar = (event) => {
      const { modulo } = event.detail;
      if (modulo === 'logistica') {
        setCurrentMainModule('logistica');
        setSelectedCategory('dashboard');
      } else if (modulo === 'recursosHumanos') {
        setCurrentMainModule('recursosHumanos');
        setSelectedCategory('dashboard-rrhh');
      }
    };

    window.addEventListener('cambiarModuloSidebar', handleCambiarModuloSidebar);
    return () => window.removeEventListener('cambiarModuloSidebar', handleCambiarModuloSidebar);
  }, []);
  ...
  const recursosHumanosCategories = [
    ...
    { id: 'reportes-rrhh', icon: '📋', title: 'Reportes', modules: ['reportes-asistencia-rrhh', 'reportes-horarios-rrhh', 'reportes-emergencias-rrhh', 'gestion-horarios-rrhh'] },
                ...
                <div className="module-selector">
                  ...
                  <button 
                    ...
                    onClick={() => {
                      setCurrentMainModule('recursosHumanos');
                      setSelectedCategory('dashboard-rrhh');
                      onModuleChange('dashboard-supervisor'); // Activa la vista de RRHH para mostrar el login
                    }}
                  >
                    👔 RECURSOS HUMANOS
                  </button>
